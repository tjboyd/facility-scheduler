...
export async function sendSignInEmail(to: string, token: string, minutes: number): Promise<void> {
  await sendMail({
    to,
    subject: `Your sign-in link — ${ORG_NAME} indoor facility`,
    text: [
      `Here is your sign-in link for the ${ORG_NAME} indoor facility scheduler.`,
      "",
      signInLink(token),
      "",
...
export async function sendInviteEmail(to: string, token: string, invitedBy: string): Promise<void> {
  await sendMail({
    ...
    text: [
      `${invitedBy} added you to the ${ORG_NAME} indoor facility scheduler.`,
      "",
